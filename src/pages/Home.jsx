      ...
      <main className="container">
        <section className="grid">
          <div className="card" style={{gridColumn:'span 12'}}>
            ...
            <p className="muted">Esta es la vista pública. La sesión persiste mientras dure la pestaña/ventana.</p>
          ...
          <div style={{gridColumn:'span 3'}}><StatCard label="Items carrito" value={session?.cart?.length ?? 0} hint="LocalStorage (sesión)"/></div>
          <div style={{gridColumn:'span 3'}}><StatCard label="Usuario" value={session?.user?.name ?? '—'} hint="Guardado por sesión"/></div>
          <div style={{gridColumn:'span 6'}} className="card">
            <h3>Tabla demo</h3>
            <table className="table-like">
              <thead><tr><th>Ítem</th><th>Cantidad</th></tr></thead>
              <tbody>
                {(session?.cart ?? []).map((it, idx)=>(
                  <tr key={it.id}><td>Producto #{idx+1}</td><td>1</td></tr>
                ))}
